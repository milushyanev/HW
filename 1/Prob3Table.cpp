...
Problem3Table::Problem3Table(char const *file, int row, int col)
{
...
void Problem3Table::calcTable(void)
{
	int size = cols * rows;
	...
	for (int i = 0; i < cols; i++)
	{
		...
		//sum must start from 0 and be added to Total
		colSum[j++] = sum;
		Total += sum;
		sum = 0;
	}
